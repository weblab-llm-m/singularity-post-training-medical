...
class Qwen3NextGatedDeltaNet(MegatronModule, _Qwen3NextGatedDeltaNet):
    ...
    def forward(self, hidden_states: torch.Tensor, **kwargs):
        ...
        use_cp = cp_size is not None and cp_size > 1
        cp_group = parallel_state.get_context_parallel_group() if use_cp else None

        
        # 1) まずは従来どおり sequence parallel を解消（TP グループ内）
        if args.sequence_parallel and args.tensor_model_parallel_size > 1:
            hidden_states = gather_from_sequence_parallel_region(hidden_states)

        # 2) CP が有効なら、CP グループ内で seq 方向 all-gather して
        #    「グローバルなシーケンス長 S_total」を持たせる
        if use_cp:
        ...
        # ==== 出力側を元の shard (CP or SP) に戻す ====
        if use_cp:
            # [global_seq, b, h] -> [local_seq, b, h]
            res = scatter_to_sequence_parallel_region(res, group=cp_group)
