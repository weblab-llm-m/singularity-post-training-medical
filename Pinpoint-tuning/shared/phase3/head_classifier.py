#!/usr/bin/env python3
"""
Head Classifier（Dense/MoE両対応）
抽出された注意パターンから3種類のヘッドを分類
翻訳論文の手法を踏襲

Dense/MoE両対応版:
- num_layers/num_headsはコンストラクタで指定（デフォルトなし）
- indicator_type で 'guideline' / 'profile' を切り替え
"""
...
class HeadClassifier:
    ...
    def __init__(
        ...
        Args:
            ...
            num_layers: レイヤー数
            num_heads: ヘッド数
            indicator_type: 'guideline' または 'profile'
                - guideline: guideline_indicator_heads / guideline_indicator_positions
                - profile: profile_indicator_heads / profile_indicator_positions
        """
        ...
        # indicator_type に応じたキー名を設定
        self._indicator_head_key = f'{indicator_type}_indicator_heads'
        self._indicator_position_key = f'{indicator_type}_indicator_positions'
        self._indicator_criteria_key = f'{indicator_type}_indicator'

        print("HeadClassifier initialized")
    ...
    def classify_all_heads(self) -> Dict:
        ...
        Returns:
            results: {
                ...
                '<indicator_type>_indicator_heads': [(layer, head), ...],
        ...
        # プログレス表示間隔を総ヘッド数に応じて自動調整
        log_interval = 200 if total_heads > 1000 else 100
        ...
        for layer in range(self.num_layers):
            for head in range(self.num_heads):
                ...
                if classified_count % log_interval == 0:
    ...
    def classify_single_head(self, layer: int, head: int) -> str:
        ...
        2. Indicator Heads (guideline/profile)
        ...
        elif self.is_indicator_head(avg_attention_pattern):
            return self._indicator_head_key
    ...
    def is_indicator_head(self, attn_pattern: torch.Tensor) -> bool:
        """
        指示語ヘッドかどうかを判定（guideline/profile両対応）

        スパイク検出: 指示語位置への注意が他位置と比べて突出しているか

        Args:
            attn_pattern: 注意パターン [seq_len]

        Returns:
            is_indicator: 指示語ヘッドかどうか
        """
        all_indicator_positions = []

        for annotation in self.annotation_data:
            positions = annotation.get(self._indicator_position_key, [])
            all_indicator_positions.extend(positions)

        if not all_indicator_positions:
            return False

        seq_len = len(attn_pattern)
        valid_positions = [p for p in all_indicator_positions if p < seq_len]

        if not valid_positions:
            return False

        max_attn_to_indicator = attn_pattern[valid_positions].max().item()
        ...
        spike_ratio = max_attn_to_indicator / (mean_other_attn + 1e-10)

        spike_threshold = self.criteria[self._indicator_criteria_key]['spike_threshold']
        spike_ratio_threshold = self.criteria[self._indicator_criteria_key]['spike_ratio']

        return (max_attn_to_indicator > spike_threshold and
    ...
    def print_classification_summary(self, results: Dict):
        ...
        indicator_key = self._indicator_head_key
        indicator_label = indicator_key.replace('_', ' ').title()
        print(f"  {indicator_label}: {len(results[indicator_key])} " +
              f"({len(results[indicator_key])/total_heads*100:.1f}%)")
...
def main():
    parser = argparse.ArgumentParser(description="Head Classifier (Dense/MoE両対応)")
    parser.add_argument(
        "--attention_patterns",
        type=str,
        required=True,
    ...
    parser.add_argument(
        ...
        type=str,
        default="guideline",
        choices=["guideline", "profile"],
        help="Indicator type: 'guideline' or 'profile' (default: guideline)"
    )
