...
Dense (qwen2, qwen3, llama, mistral) と MoE (qwen3_moe) の両方に対応。
'''
...
# MoE用関数は存在する場合のみimport
try:
    from utils.utils_spt import freeze_modules_moe, freeze_lora_modules_moe
    _HAS_MOE = True
except ImportError:
    _HAS_MOE = False
...
def main():
    ...
    if args.precise_level > 0 and args.path_patching_path is not None:
        ...
        if args.model_type == 'qwen3_moe':
            if not _HAS_MOE:
                raise ImportError("MoE freeze functions not found in utils.utils_spt. "
                                  "Ensure freeze_modules_moe/freeze_lora_modules_moe are defined.")
        ...
        # Dense モデルの場合 (qwen2, qwen3, llama, mistral)
