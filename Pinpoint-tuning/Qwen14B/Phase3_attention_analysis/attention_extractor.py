...
class AttentionExtractor:
    ...
    def __init__(self, model, num_layers: int, num_heads: int):
        ...
        print(f"AttentionExtractor initialized")
    ...
    def extract_and_save(
        ...
        # データを整形
        # {layer_idx: [batch, num_heads, seq_len]} のリストを
        # [num_layers, batch, num_heads, seq_len] に変換
        formatted_patterns = {}
        ...
        if save_dir:  # ディレクトリパスが空でない場合のみ作成
    ...
    def print_attention_summary(self, attention_patterns: Dict[int, torch.Tensor]):
        ...
        """
        print("\n" + "="*60)
        print("Attention Patterns Summary")
        print("="*60)

        for layer_idx in sorted(attention_patterns.keys())[:5]:  # 最初の5層のみ
