#!/bin/bash
# SPT Training for 321 Important Heads on ACS Data
# Same configuration as Phase5_v2_megatron_config
...
echo "=========================================================================="
echo "SPT Training - 321 Important Heads on ACS Data (Megatron-compatible)"
echo "=========================================================================="
echo ""
...
echo "  Samples: 9,200 (ACS Data v1)"
...
echo "  Global batch size: 128 (Megatron)"
echo "  Epochs: 1 (Megatron)"
echo "  Max sequence length: 2048 (data coverage: 100%)"
...
# SPT configuration
TRAINABLE_HEADS="${BASE_DIR}/Phase5_v5_pinpoint_tuning_mk2_8gpu/trainable_heads_all_321.json"
OUTPUT_DIR="${BASE_DIR}/Phase5_v5_pinpoint_tuning_mk2_8gpu/spt_321heads_acs_output"
TRAIN_DATA="${DATA_DIR}/train.parquet"
CACHE_DIR="${BASE_DIR}/Phase5_v5_pinpoint_tuning_mk2_8gpu/cache"
...
PRECISE_LEVEL=3  # Only qkv_proj
LEARNING_RATE=2e-5  # Megatron
BATCH_SIZE=1
GRADIENT_ACCUMULATION=128  # Global batch size = 128 (Megatron)
NUM_EPOCHS=1  # Megatron
MAX_SEQ_LENGTH=2048  # Covers 100% of data
WEIGHT_DECAY=0.01  # Megatron
ADAM_BETA1=0.9  # Megatron
ADAM_BETA2=0.95  # Megatron
MAX_GRAD_NORM=1.0  # Megatron
WARMUP_RATIO=0.1  # Megatron
SAVE_STEPS=100
LOGGING_STEPS=10
...
echo "  5. Max sequence length: 2048 (optimal for data)"
echo "  6. dataloader_num_workers: 2 (from Phase5 success)"
echo ""
...
# Build training command (following Phase5 pattern)
TRAIN_CMD="python3 ${BASE_DIR}/Phase5_v5_pinpoint_tuning_mk2_8gpu/run_spt_medical.py"
...
# Execute training on SINGLE GPU (following Phase5 successful pattern)
...
# Set PYTHONPATH to include Phase5_v5_pinpoint_tuning_mk2_8gpu directory
export PYTHONPATH="${BASE_DIR}/Phase5_v5_pinpoint_tuning_mk2_8gpu:$PYTHONPATH"
