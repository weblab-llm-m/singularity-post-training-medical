#!/bin/bash
# SPT Training for Qwen3-30B-A3B-Instruct-2507 (MoE) - 8 GPU Version
# Uses DeepSpeed ZeRO-3 for memory-efficient training

set -e

echo "==================================================="
echo "SPT Training for Qwen3-30B-A3B MoE (8 GPU)"
echo "==================================================="
...
OUTPUT_DIR="${BASE_DIR}/Phase5_pinpoint_tuning/spt_output_8gpu"
TRAIN_DATA="${DATA_DIR}/train.parquet"
CACHE_DIR="${BASE_DIR}/Phase5_pinpoint_tuning/cache"

# Training parameters (optimized for 8 GPU distributed training)
USE_LORA=false
PRECISE_LEVEL=4  # Only qkv_proj
LEARNING_RATE=1e-4
BATCH_SIZE=1
GRADIENT_ACCUMULATION=4  # Lower accumulation with 8 GPUs
NUM_EPOCHS=2
MAX_SEQ_LENGTH=1024  # Can use longer sequences with 8 GPUs
SAVE_STEPS=100
LOGGING_STEPS=10
