...
# -*- coding: utf-8 -*-
'''
Model Evaluation Script for Medical QA (MoE version)
Evaluates model accuracy on gynecology guideline dataset
Optimized for Qwen3-30B-A3B-Instruct-2507 MoE model
'''
...
def evaluate_model(
    ...
    use_flash_attn : bool, optional
        Whether to use flash attention (requires flash-attn package)
    ...
    print("Note: This is a 30B MoE model, loading may take some time...")
    ...
    if hasattr(model.config, 'num_experts'):
        print(f"MoE model loaded:")
        print(f"  - Experts: {model.config.num_experts}")
        print(f"  - Active per token: {getattr(model.config, 'num_experts_per_tok', 'N/A')}")
    ...
    with torch.no_grad():
        for idx, row in tqdm(df.iterrows(), total=len(df), desc="Evaluating"):
            ...
            outputs = model.generate(
                input_ids,
                max_new_tokens=128,  # Reduced for MoE memory efficiency
                do_sample=False,
                temperature=None,
                top_p=None,
    ...
    if metrics['num_experts']:
        print(f"  MoE experts: {metrics['num_experts']} (active: {metrics['num_experts_per_tok']})")
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate MoE model on medical QA')
