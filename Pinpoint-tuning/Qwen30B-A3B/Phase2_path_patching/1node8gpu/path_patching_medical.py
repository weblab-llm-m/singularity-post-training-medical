...
def path_patching_batch(
    ...
    return results, expert_analysis
...
def main():
    ...
    for i in tqdm(range(0, sample_num, batch_size), desc="Batches"):
        ...
        per_sample_results.append(batch_results.detach().cpu())
        cnt += len(batch_data)
    ...
    # Stack per-sample results: (num_samples, num_layers, num_heads)
    per_sample_tensor = torch.stack(per_sample_results)
    # Averaged results for backward compatibility
    results = per_sample_tensor.mean(dim=0).to(model.device)
    ...
    # Save per-sample results: (num_samples, num_layers, num_heads)
    per_sample_path = os.path.join(output_dir, "results_per_sample.pt")
    torch.save(per_sample_tensor, per_sample_path)
    print(f"Per-sample results saved to: {per_sample_path} (shape: {per_sample_tensor.shape})")
    ...
    print(f"  - results.pt (averaged: {results.detach().cpu().shape})")
    print(f"  - results_per_sample.pt (per-sample: {per_sample_tensor.shape})")
