...
# dictionary_260212 (5,003件) を使用
#
# Usage:
#   bash submit_all.sh overlap_medical    # 5003件 (医療用語置換)
#   bash submit_all.sh overlap_reasoning  # 5003件 (推論キーワード置換)
# =============================================================
set -e
...
SCRIPT_DIR="${BASE_DIR}/Phase2_path_patching/8node64gpu_260212"
DICT_DIR="${BASE_DIR}/Phase1_data_preparation/dictionary_260212"

if [ "${DATASET}" = "overlap_medical" ]; then
    DATA_PATH="${DICT_DIR}/path_patching_data_overlap_medical.jsonl"
    OUTPUT_DIR="${SCRIPT_DIR}/results_overlap_medical"
elif [ "${DATASET}" = "overlap_reasoning" ]; then
    DATA_PATH="${DICT_DIR}/path_patching_data_overlap_reasoning.jsonl"
...
echo "Dictionary: dictionary_260212 (5,003件)"
...
for node_idx in $(seq 0 $((NUM_NODES - 1))); do
    ...
    JOB_ID=$(sbatch \
        --job-name="pp260212_${DATASET}_n${node_idx}" \
        --partition=P08317 \
        --nodelist=${NODE} \
        --gres=gpu:8 \
        --ntasks=1 \
...
echo ""
echo "両データセット実行例:"
echo "  bash ${SCRIPT_DIR}/submit_all.sh overlap_medical    # 5003件 (医療用語置換)"
echo "  bash ${SCRIPT_DIR}/submit_all.sh overlap_reasoning  # 5003件 (推論キーワード置換)"
echo "==================================================="
