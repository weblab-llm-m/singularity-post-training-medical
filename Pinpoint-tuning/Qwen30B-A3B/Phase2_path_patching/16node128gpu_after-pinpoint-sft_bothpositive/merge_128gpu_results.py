#!/usr/bin/env python3
"""
128並列Path Patching結果を統合するスクリプト (positive_pinpoint)

16ノード × 8GPU = 128プロセスの結果を重み付け平均で統合。
サンプル数はログファイルから自動取得。
"""
...
def merge_results(results_dir: str, num_processes: int = 128):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Merge 128-GPU parallel path patching results (positive_pinpoint)")
    parser.add_argument("results_dir", type=str,
                        help="Directory containing process_XXX/results.pt")
    parser.add_argument("--num_processes", type=int, default=128)
    args = parser.parse_args()

    print("=" * 60)
    print("Merging 128-GPU Parallel Path Patching Results (positive_pinpoint)")
    print("=" * 60)
