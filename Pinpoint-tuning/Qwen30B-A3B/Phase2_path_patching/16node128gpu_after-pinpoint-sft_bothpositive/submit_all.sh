#!/bin/bash
# =============================================================
# Phase 2: Path Patching - 16ノード128GPU並列実行 (SLURM)
# モデル: positive_pinpoint (Pinpoint-SFT後モデル)
# データ: overlap (4,393件)
#
# Usage:
#   bash submit_all.sh overlap_medical    # 4393件 (医療用語置換)
#   bash submit_all.sh overlap_reasoning  # 4393件 (推論キーワード置換)
# =============================================================
set -e

DATASET=${1:?"Usage: bash submit_all.sh [overlap_medical|overlap_reasoning]"}

BASE_DIR="/home/yuuki.nakamura/singularity-post-training-medical/Pinpoint-tuning/Qwen30B-A3B"
SCRIPT_DIR="${BASE_DIR}/Phase2_path_patching/16node128gpu_after-pinpoint-sft_bothpositive"
if [ "${DATASET}" = "overlap_medical" ]; then
    ...
    echo "ERROR: Unknown dataset '${DATASET}'. Use 'overlap_medical' or 'overlap_reasoning'."
    exit 1
fi
...
NODES=(osk-gpu13 osk-gpu22 osk-gpu46 osk-gpu47 osk-gpu48 osk-gpu49 osk-gpu50 osk-gpu51 osk-gpu52 osk-gpu53 osk-gpu61 osk-gpu65 osk-gpu66 osk-gpu67 osk-gpu68 osk-gpu69)

echo "==================================================="
echo "Phase 2: Path Patching - 16 Node × 8 GPU = 128 Parallel"
echo "Model:      positive_pinpoint (Pinpoint-SFT後)"
echo "Data:       overlap (4,393件)"
...
for node_idx in $(seq 0 $((NUM_NODES - 1))); do
    ...
    JOB_ID=$(sbatch \
        --job-name="DONT-KILL_MI_${DATASET}_n${node_idx}" \
        --partition=P08317 \
        --nodelist=${NODE} \
        --gres=gpu:8 \
        --ntasks=1 \
    ...
    JOB_IDS+=($JOB_ID)
    echo "  Node ${node_idx} (${NODE}): JobID ${JOB_ID} (process_$(printf '%03d' $((node_idx*8)))-$(printf '%03d' $((node_idx*8+7))))"
done
...
echo "  bash ${SCRIPT_DIR}/monitor_all_nodes.sh ${DATASET}  # 進捗確認"
echo ""
echo "完了後のマージ:"
echo "  ${BASE_DIR}/venv/bin/python3 ${SCRIPT_DIR}/merge_128gpu_results.py ${OUTPUT_DIR}"
echo ""
echo "両データセット実行例:"
echo "  bash ${SCRIPT_DIR}/submit_all.sh overlap_medical    # 4393件 (医療用語置換)"
echo "  bash ${SCRIPT_DIR}/submit_all.sh overlap_reasoning  # 4393件 (推論キーワード置換)"
echo "==================================================="
