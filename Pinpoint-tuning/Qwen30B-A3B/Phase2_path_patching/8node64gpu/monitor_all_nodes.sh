#!/bin/bash
# =============================================================
# 8ノード64GPU並列 Path Patching モニタリング
# =============================================================

BASE_DIR="/home/yuuki.nakamura/singularity-post-training-medical/Pinpoint-tuning/Qwen30B-A3B"
OUTPUT_DIR="${BASE_DIR}/Phase2_path_patching/8node64gpu/results"
...
echo "==================================================="
echo "Path Patching Monitor - 8 Nodes × 8 GPUs"
echo "Time: $(date)"
echo "==================================================="
echo ""
...
# SLURM jobs status
echo "--- SLURM Jobs ---"
squeue -u $(whoami) --format="%.10i %.12j %.8T %.10M %.6D %R" 2>/dev/null | grep -E "JOBID|pp_n"
echo ""
...
if [ ${COMPLETED} -eq ${TOTAL_GPUS} ]; then
    ...
    echo "Run merge:"
    echo "  ${BASE_DIR}/venv/bin/python3 ${BASE_DIR}/Phase2_path_patching/8node64gpu/merge_64gpu_results.py ${OUTPUT_DIR}"
fi
