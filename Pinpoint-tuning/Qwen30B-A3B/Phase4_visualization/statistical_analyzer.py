...
def analyze_layer_groups(classification_results: dict, num_layers: int = 48) -> dict:
    ...
    for group_name, layer_indices in groups.items():
        ...
        for head_type in ['medical_term_heads', 'profile_indicator_heads', 'reasoning_flow_heads']:
...
def main():
    ...
    for group, stats in group_analysis.items():
        total = sum(stats[k] for k in ['medical_term_heads', 'profile_indicator_heads', 'reasoning_flow_heads'])
