...
def generate_layer_distribution_plot(
    ...
    profile_counts = np.zeros(num_layers)
    reasoning_counts = np.zeros(num_layers)

    for l, h in classification_results.get('medical_term_heads', []):
        if l < num_layers:
            medical_counts[l] += 1
    for l, h in classification_results.get('profile_indicator_heads', []):
        if l < num_layers:
            profile_counts[l] += 1
    ...
    ax.bar(layers, profile_counts, width, label='Profile Indicator', color='blue', alpha=0.7)
